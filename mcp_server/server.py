...
HORARIO_ABERTURA = 9   # 09:00
HORARIO_FECHAMENTO = 19  # 19:00
DURACAO_CORTE_MIN = 30  # minutos por slot
...
async def listar_horarios_disponiveis(params: ListarHorariosInput) -> str:
    ...
    Considera agendamentos já marcados e calcula os slots livres com base na
    duração padrão de 30 minutos por serviço.

    Args:
        params: ListarHorariosInput com data no formato YYYY-MM-DD.

    Returns:
        JSON com: data, horarios_disponiveis (list de strings HH:MM),
        horarios_ocupados (list de dicts com hora e servico).
    """
    conn = get_db()
    try:
        agendados = rows_to_list(conn.execute(
            "SELECT data_hora, servico FROM agendamentos WHERE data_hora LIKE ? AND status NOT IN ('cancelado')",
            (f"{params.data}%",)
        ).fetchall())

        ocupados_horas = {a["data_hora"][11:16] for a in agendados}

        todos_slots = []
        hora_atual = datetime.strptime(f"{params.data} {HORARIO_ABERTURA:02d}:00", "%Y-%m-%d %H:%M")
        hora_fim = datetime.strptime(f"{params.data} {HORARIO_FECHAMENTO:02d}:00", "%Y-%m-%d %H:%M")

        while hora_atual < hora_fim:
            todos_slots.append(hora_atual.strftime("%H:%M"))
            hora_atual += timedelta(minutes=DURACAO_CORTE_MIN)

        disponiveis = [s for s in todos_slots if s not in ocupados_horas]
        ocupados_fmt = [{"hora": a["data_hora"][11:16], "servico": a["servico"]} for a in agendados]

        return json.dumps({
            "data": params.data,
            "horarios_disponiveis": disponiveis,
            "horarios_ocupados": ocupados_fmt,
...
async def agendar_corte(params: AgendarCorteInput) -> str:
    ...
    Verifica se o horário está disponível antes de confirmar.
    Retorna o agendamento criado ou erro se o horário já estiver ocupado.
    ...
    Returns:
        JSON com: sucesso (bool), agendamento (dict com id, data_hora, servico, status)
        ou mensagem de erro.
    """
    conn = get_db()
    try:
        ...
        conflito = conn.execute(
            "SELECT id FROM agendamentos WHERE data_hora = ? AND status NOT IN ('cancelado')",
            (data_hora,)
        ).fetchone()
        ...
        if conflito:
            return json.dumps({
                "sucesso": False,
                "erro": f"Horário {params.hora} do dia {params.data} já está ocupado. Escolha outro horário."
            }, ensure_ascii=False)

        cliente = conn.execute("SELECT nome FROM clientes WHERE id = ?", (params.cliente_id,)).fetchone()
        if not cliente:
        ...
        conn.execute(
            ...
            (novo_id, params.cliente_id, data_hora, params.servico, "confirmado", params.observacoes or "", agora)
        )
        # Atualiza última visita e total de visitas
        conn.execute(
